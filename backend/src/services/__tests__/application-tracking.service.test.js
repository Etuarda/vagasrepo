...
describe("application tracking from analysis", () => {
  const analysis = {
    ...
    matchScore: 82,
    globalScore: 70,
    selectedProfileScore: 82,
    selectedProfileName: "Backend",
    selectedProfileType: "subprofile",
    skillsScore: 90,
    projectsScore: 65,
    ...
    selectedCourseIds: ["course"],
    selectedCertificationIds: [],
    warnings: [],
    scoringVersion: "ats-v5-skills-projects",
    status: "draft",
    appliedAt: null,
  };
  const payload = {
  ...
  it("cria candidatura ativa vinculada e alinha a fase ao gerar curriculo", async () => {
    ...
    expect(tx.job.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({
        ...
        matchingSnapshot: expect.objectContaining({
          analysisId: "analysis",
          overallScore: 82,
          matchedSkills: ["Node.js", "PostgreSQL"],
          missingSkills: ["Redis"],
